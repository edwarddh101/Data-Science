...
* Evaluation (these apply to all classification algorithms)
  ...
  * Receiver Operator Characteristic (ROC) plot - good to see sensitivity and False positve rate to
    threshold parameter
...
* Interpretability
  * SVMs don't output a probability of classification nor do they have
    ...
    regression or random forests (assuming use of kernels).
* Communication
...
transformations are not an issue, and they are immune to the effects of predictor outliers.
    * A single tree can be led astray by outliers, however an ensemble using bootstrap bagging such as Random Forest can diminish the effects of the outlier
* Overfitting
...
* Nonlinearity/power
    * Random forest reduces the correlation between the trees without increasing the variance too much through a random selection of the input variables. However, not all estimators can be improved by shaking up the data like this. It seems that highly nonlinear estimators, such as trees, benefit the most.
* n << p
...
* Evaluation
    ...
    * The effect of Adaboost is to reduce bias. As the weak classifier learns the accuracy improves. 
* Nonlinearity/power
* n << p
* Outliers
